...
Remote (Streamable HTTP / Cloud Run) deployment of the pharmacy server and
the Wireshark protocol analysis are scoped for the second delivery.
...
# Official Git MCP server (not on PyPI as part of this repo's own deps —
# it is a third-party server this host connects to, not a dependency of it)
...
and disables `pharmacy-remote` (used in the second delivery).
...
tests/         unit tests (protocol, pharmacy tools) + a stdio integration test
...
enforcement, stock decrements), and one end-to-end integration test that
spawns the real pharmacy stdio server as a subprocess and drives it through
